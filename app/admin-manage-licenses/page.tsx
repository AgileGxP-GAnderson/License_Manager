...
import PurchaseOrderForm from "@/components/purchase-order-form"; // +++ Import PurchaseOrderForm
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function AdminManageLicensesPage() {
  ...
  const {
    ...
    isLoadingPurchaseOrders, // Corrected: was 'loading'
    purchaseOrderError,    // Corrected: was 'error'
  ...
  const [isAddPOModalOpen, setIsAddPOModalOpen] = useState(false); // +++ State for PO Form Modal
  ...
  useEffect(() => {
    ...
    if (customerId) {
      ...
      usePurchaseOrderStore.setState({ purchaseOrders: [], purchaseOrderError: null }); // Corrected: was 'error'
      console.log("No customer selected, clearing/not fetching purchase orders.");
    }
  }, [selectedCustomer?.id, fetchPurchaseOrdersByCustomerId]);

  const handleAddPOSuccess = () => {
    setIsAddPOModalOpen(false);
    if (selectedCustomer?.id) {
      fetchPurchaseOrdersByCustomerId(selectedCustomer.id); // Refresh PO list
    ...
    <Dialog open={isAddPOModalOpen} onOpenChange={setIsAddPOModalOpen}>
        ...
        {selectedCustomer && !isLoadingCustomer && !customerError && (
          <Card className="enhanced-card">
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button onClick={() => setIsAddPOModalOpen(true)} size="sm">
            ...
            <CardContent className="pt-6">
              ...
              // Render the PurchaseOrderList component (ensure it exists and path is correct)
                ...
                <>
                  {purchaseOrders.length === 0 && !isLoadingPurchaseOrders && (
                    <div className="text-center text-gray-500 py-4">
                      <p>No purchase orders found for this customer.</p>
                      {/* REMOVED 'Create First Purchase Order' button */}
                    </div>
                  )}
                  {purchaseOrders.length > 0 && (
                    <PurchaseOrderList
                      ...
                      isAdminView={true} // Pass isAdminView prop
                    />
                  )}
                </>
              )}
      ...
      <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Fill in the details for the new purchase order. Click save when you&apos;re done.
            </DialogDescription>
          </DialogHeader>
          {selectedCustomer && (
              <PurchaseOrderForm
                  customerId={selectedCustomer.id}
                  onSuccess={handleAddPOSuccess}
                  onCancel={() => setIsAddPOModalOpen(false)}
              />
          )}
        </DialogContent>
    </Dialog>
